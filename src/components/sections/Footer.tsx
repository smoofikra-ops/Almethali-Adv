...
const SocialIconWrapper = ({ children, href }: { children: React.ReactNode, href: string }) => (
  <a href={href} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-brand-teal-deep/30 rounded-xl flex items-center justify-center hover:scale-110 transition-transform shadow-sm hover:shadow-md text-white hover:bg-brand-teal-deep/60 border border-brand-teal-deep/50">
    {children}
  </a>
);
...
const FooterAccordion = ({ title, children, isOpen, onToggle }: { title: string, children: React.ReactNode, isOpen: boolean, onToggle: () => void }) => {
  return (
    <div className="border-b border-brand-teal-deep/50 md:border-none md:pb-0 pb-3">
      <button 
        onClick={onToggle} 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 md:gap-12 mb-16">
              ...
              >
                <ul className="space-y-3 pt-2 md:pt-0">
                  {servicesConfig.categories.filter(c => c.enabled).sort((a, b) => a.order - b.order).map((cat, idx) => (
                    <li key={idx}>
                      <a href={`#service-${cat.id}`} className="hover:text-brand-cyan transition-colors text-sm block">
            ...
            <div className="lg:col-span-2">
              ...
              >
                <ul className="space-y-3 pt-2 md:pt-0">
                  <li><a href="#/policies/privacy" className="hover:text-brand-cyan transition-colors text-sm">{t.footer.privacy}</a></li>
                  <li><a href="#/policies/terms" className="hover:text-brand-cyan transition-colors text-sm">{t.footer.terms}</a></li>
                  <li><a href="#/policies/usage" className="hover:text-brand-cyan transition-colors text-sm">{t.footer.usage}</a></li>
                  <li><a href="#/policies/warranty" className="hover:text-brand-cyan transition-colors text-sm">{t.footer.warranty}</a></li>
                </ul>
              </FooterAccordion>
            </div>
              ...
              >
                <ul className="space-y-4 pt-2 md:pt-0">
                  <li className="flex items-start gap-3">
                    <MapPin className="w-5 h-5 text-brand-teal-soft shrink-0 mt-0.5" />
                  ...
                  <li className="pt-2">
                    <a 
                      ...
                      className="inline-flex items-center justify-center gap-2 w-full px-4 py-2 bg-accent text-white rounded-lg hover:bg-accent-deep transition-colors text-sm font-bold"
                    >
          ...
          <div className="border-t border-brand-teal-deep/50 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-4 items-center">
               <a 
                 ...
                 target="_blank" 
                 rel="noopener noreferrer" 
                 className="text-xs text-white/40 hover:text-brand-cyan/60 transition-colors flex items-center gap-1"
               >
                 {t.footer.madeWith} <span className="text-brand-teal-soft">❤</span> NmoLabs
               </a>
            </div>
          </div>
        </div>
