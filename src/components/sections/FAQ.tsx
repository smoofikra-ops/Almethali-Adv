    ...
    >
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div variants={itemVariants} className="text-center mb-12 md:mb-16">
          <h2 className="text-3xl md:text-4xl font-display font-bold text-text-primary mb-4">الأسئلة الشائعة</h2>
          <p className="text-lg text-text-secondary">إجابات وافية لأهم استفسارات عملائنا.</p>
        </motion.div>
          ...
          {visibleFaqs.map((faq, idx) => (
            ...
            >
              <button
                ...
                className="w-full text-right px-5 py-4 md:px-6 md:py-5 flex items-center justify-between font-bold text-base md:text-lg text-text-primary focus:outline-none"
              >
                {faq.question}
              ...
              >
                <p className="text-text-secondary leading-relaxed text-sm md:text-base">{faq.answer}</p>
              </div>
            </motion.div>
          ))}
        </div>
